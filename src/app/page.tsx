    ...
    <main className="p-6 space-y-8">
      ...
      <section className="p-4 border rounded space-y-3">
        ...
        <p className="text-sm text-gray-600">
          Recommendation = oldest <code>lastPurchase</code> (ties randomized).
          Override anytime.
        </p>
          ...
          >
            ...
            {users.map((u) => (
              <option key={u.id} value={u.id}>
                {u.userName} — last: {new Date(u.lastPurchase).toLocaleString()}{" "}
                — total: {u.totalPurchases}
              </option>
            ))}
          </select>
            ...
            {users.map((u) => (
              <tr key={u.id} className="odd:bg-white even:bg-gray-500">
                ...
                <td className="p-2 border-b">
                  {new Date(u.lastPurchase).toLocaleString()}
                </td>
              </tr>
            ))}
          </tbody>
